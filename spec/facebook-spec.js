describe('facebook event fetcher', function() {
	var https = require('https');
	var fb = require('../facebook-aggregator.js');
	...
	it('exports a query function', function() {
		expect(typeof(fb.getFacebookEvents)).toBe('function');
	});
	
	it('submits an HTML request', function() {
		spyOn(https, 'request').and.callThrough();
		
		fb.getFacebookEvents(token);
	...
	it('returns a JSON string of valid events', function(done) {
		...
		fb.getFacebookEventsEvents(token, callback);//
